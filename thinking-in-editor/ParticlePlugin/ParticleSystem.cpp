...
void DemoFire::onEnter()
{
    ...
    _emitter->setPosition( Point(10,200) );
...
void DemoGalaxy::onEnter()
{
    ...
    _emitter->setTexture( Director::getInstance()->getTextureCache()->addImage(s_fire) );
	_emitter->setPosition(Point(300, 200));
    setEmitterPosition(); 
}
...
{
    _index = (_index + 1) % 4;
	auto rect = Rect(_index * 32.0f, 0.0f, 32.0f, 32.0f);
...
void ParticleDemo::onEnter(void)
{
    ...
    _color = LayerColor::create( Color4B(30,45,80,255) );
	...
	/*
    auto item4 = MenuItemToggle::createWithCallback( CC_CALLBACK_1(ParticleDemo::toggleCallback, this),MenuItemFont::create( "Free Movement" ),MenuItemFont::create( "Relative Movement" ),MenuItemFont::create( "Grouped Movement" ),NULL );
	...
	*/
    ...
    // moving background
    _background = Sprite::create("Images//pool.png");
	this->addChild(_background, 5);
    _background->setPosition( Point(300, s.height-300) );
...
void ParticleBatchHybrid::onEnter()
{
	...
	auto s = Director::getInstance()->getWinSize();
	_emitter->setPosition(Point(s.width / 2-120, s.height / 2+80));
	_emitter->setScale(0.1f);
...
void MultipleParticleSystems::onEnter()
{
    ...
    for (int i = 0; i<5; i++) {
		...
		particleSystem->setPosition(Point(i*50.0f, i * 50.0f));
...
void AddAndDeleteParticleSystems::onEnter()
{
    ...
    for (int i = 0; i<6; i++) {
		...
		particleSystem->setPosition(Point(i*15.0f + 100.0f, i * 15.0f + 100.0f));
...
void AddAndDeleteParticleSystems::removeSystem(float dt)
{
    ...
    if (nChildrenCount > 0) 
    {
		...
		particleSystem->setPosition(Point((float)(rand() % 300), (float)(rand() % 400)));
...
void ReorderParticleSystems::onEnter()
{
    ...
    for (int i = 0; i<3; i++) {
		...
		particleSystem->setPosition(Point(i * 10 + 120.0f, 200.0f));
